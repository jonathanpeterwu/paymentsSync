...
var SYNAPSE_API = 'https://synapsepay.com/api/3/',
    SYNAPSE_SANDBOX = 'https://sandbox.synapsepay.com/api/3';
...
module.exports = function(publicKey, privateKey, sandbox, platform) {
  ...
  var getDocumentId = function(data) {
  ...
  return {
    ...
    dwolla: {
      ...
      funding: {
        ...
        verify: function(data, cb) {
          sendPost('/customers/' + getId(data) + '/iav-token', data, cb);
        }
      },
      transfer: {
        create: function(data,cb) {
      ...
      document: {
        create: function(data, cb) {
          sendPost('/customers/' + getId(data) + '/documents', data, cb);
        },
        info: function(data, cb) {
          sendGet('/customers/' + getId(data) + '/documents' + getDocumentId(data), cb);
        },
        list: function(data, cb) {
          sendGet('/customers/' + getId(data) + '/documents', cb);
        }
      }
    },
    plaid: {
    ...
    synapse: {
      account: {
        create: function(data,cb) {
          sendPost('/users', data, cb);
        },
        info: function(data, cb) {
          sendGet('/users/' + getId(data), cb);
        },
        update: function(data, cb) {
          sendPatch('/users/' + getId(data), data, cb);
        },
        document: {
          virtual: function(data, cb) {
            sendPost('/users/' + getId(data), data, cb);
          },
          upload: function(data, cb) {
            sendPost('/users/' + getId(data), data, cb);
          }
        },
        verification: function(data, cb) {
          sendPost('/users/' + getId(data), data, cb);
        }
      },
      transaction: {
