module.exports = function(exports) {
     ...
     * @param {String}     scope
     **/
    exports.authUrl = function(redirect_uri, scope, verified_account) {
      if (!vars._client_id) { throw new Error('Missing arg client_id'); }
      if (!scope) { scope = "Send|Transactions|Balance|Request|Contacts|AccountInfoFull|Funding|ManageAccount|Scheduled"; }
    ...
    /**
     * Use this method to retrieve the endpoints that the current/passed
     * OAuth tokens are elligible to use. 
     *
     * @param {String}   token
     * @param {Function} fn
     */
    exports.catalog = function(token, fn) {
      if (!vars._token && !token) { throw new Error('Missing arg token'); }

      var host = exports.sandbox ? 'uat.dwolla.com' : 'www.dwolla.com';
      var url = util.format("%s%s%s", 'https://', host, '/oauth/rest/catalog');

      var params = {};
      params.oauth_token = token ? token : vars._token;

      restler.get(url, {query: params})
        .on('complete', function(result, response) {
          if (result instanceof Error) fn(result.message);
          else fn(null, result._links);
        });
    }
};
